			     ...
			     struct dtl_flow *flow)
{
	...
	struct net_conf *nc;
	long timeout = HZ;
# 5 "/scrap/drbd/drbd/build-5.15.180-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
# 1118 "/scrap/drbd/drbd/drbd_transport_lb-tcp.c"

	socket->sk->sk_reuse = SK_CAN_REUSE; /* SO_REUSEADDR */
# 11 "/scrap/drbd/drbd/build-5.15.180-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
...
# 19 "/scrap/drbd/drbd/build-5.15.180-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
# 1642 "/scrap/drbd/drbd/build-5.15.180-mdl+/drbd_transport_lb-tcp.c"
	struct msghdr msg = { .msg_flags = msg_flags | MSG_NOSIGNAL };
# 20 "/scrap/drbd/drbd/build-5.15.180-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
# 1639 "/scrap/drbd/drbd/drbd_transport_lb-tcp.c"
	struct drbd_transport *transport = &dtl_transport->transport;
	struct socket *socket = flow->socket;
# 23 "/scrap/drbd/drbd/build-5.15.180-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
...
# 33 "/scrap/drbd/drbd/build-5.15.180-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
# 1658 "/scrap/drbd/drbd/build-5.15.180-mdl+/drbd_transport_lb-tcp.c"
		sent = socket->ops->sendpage(socket, page, offset, len,
					     msg.msg_flags);
# 35 "/scrap/drbd/drbd/build-5.15.180-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
# 1652 "/scrap/drbd/drbd/drbd_transport_lb-tcp.c"
		if (sent <= 0) {
			if (sent == -EAGAIN) {
				if (drbd_stream_send_timed_out(transport, flow->stream_nr))
