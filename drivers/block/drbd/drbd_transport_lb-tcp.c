...
# 5 "/scrap/drbd/drbd/build-5.15.167-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
...
# 8 "/scrap/drbd/drbd/build-5.15.167-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
# 1610 "/scrap/drbd/drbd/build-5.15.167-mdl+/drbd_transport_lb-tcp.c"
	struct msghdr msg = { .msg_flags = msg_flags | MSG_NOSIGNAL };
# 9 "/scrap/drbd/drbd/build-5.15.167-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
# 1607 "/scrap/drbd/drbd/drbd_transport_lb-tcp.c"
	struct drbd_transport *transport = &dtl_transport->transport;
	struct socket *socket = flow->socket;
# 12 "/scrap/drbd/drbd/build-5.15.167-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
...
# 22 "/scrap/drbd/drbd/build-5.15.167-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
# 1622 "/scrap/drbd/drbd/build-5.15.167-mdl+/drbd_transport_lb-tcp.c"
		sent = socket->ops->sendpage(socket, page, offset, len,
					     msg.msg_flags);
# 24 "/scrap/drbd/drbd/build-5.15.167-mdl+/.patches/drbd_transport_lb-tcp.c.patch"
# 1620 "/scrap/drbd/drbd/drbd_transport_lb-tcp.c"
		if (sent <= 0) {
			if (sent == -EAGAIN) {
				if (drbd_stream_send_timed_out(transport, flow->stream_nr))
