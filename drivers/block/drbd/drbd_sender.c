...
{
	struct fifo_buffer *plan;
	struct block_device *bdev = bdev_whole(peer_device->device->ldev->backing_bdev);
	//struct hd_struct *part = &bdev->bd_disk->part0;
	...
	peer_device->rs_last_events = (int)part_stat_read(bdev, sectors[0])
		+ (int)part_stat_read(bdev, sectors[1]);
